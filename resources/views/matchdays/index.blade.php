                        ...
                        <thead class="bg-slate-50 border-b border-slate-200">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-xs font-semibold text-slate-500 uppercase whitespace-nowrap">
                                    Status</th>
                                <th
                                    class="px-4 py-3 text-xs font-semibold text-slate-500 uppercase text-center whitespace-nowrap">
                                    Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-slate-100 text-sm">
                            @forelse($matchdays as $matchday)
                                <tr class="hover:bg-slate-50/50">
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-slate-700">{{ $matchday->nama_matchday }}
                                    </td>
                                        ...
                                        @if(!empty($matchday->fasilitas) && is_array($matchday->fasilitas))
                                            <div class="flex flex-wrap gap-1">
                                                @foreach($matchday->fasilitas as $item)
                                                    <span
                                                        class="inline-flex items-center bg-blue-100 text-blue-800 text-[11px] font-medium px-2 py-0.5 rounded whitespace-nowrap">
                                                        {{ $item }}
                                                    </span>
                                                @endforeach
                                            </div>
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-center">
                                        <div class="flex justify-center items-center gap-2">
                                            <a href="{{ route('matchdays.peserta', $matchday->id) }}"
                                                class="text-emerald-600 hover:text-emerald-900 font-semibold text-xs bg-emerald-50 px-3 py-1.5 rounded-md transition">
                                                Peserta
                                            </a>
